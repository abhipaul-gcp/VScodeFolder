# Working Copy with new file scan
import base64
...
def hello_pubsub(cloud_event):
    try:
        ...
        ex_tag = bucket_tag.get("dlp_scan")

        if ex_tag and ex_tag == "false":
            print("Bucket exception tag found: ", ex_tag)
        else:
            print("No DLP exceptation tag found.")

            create_dlp_job(bucket_name, project_id)
    except Exception as e:
        print(f'Error in DLP jon function : {e}')
...
def create_dlp_job(bucket_name, project_id):
    ...
    schedule = {'recurrence_period_duration': '2592000s'} # Schedule every 30 Days
